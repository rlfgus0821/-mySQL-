...
-- distinct 키워드 사용 시 중복된 행을 하나만 제시
...
-- 가격이 20000 이상 25000 이하인 도서명, 가격, 저자 조회
select bookName, bookPrice, bookAuthor 
	from book 
    where bookPrice >= 20000 and bookPrice <= 25000 ;


-- 출판일이 2022년1월1일 이후인 도서명,출간일 조회
select bookName, bookDate 
	from book 
...
-- 출판사번호가 2과 3인 도서의 정보 조회
select * from book
	where pubNo in ('2','3');
    
-- select * from book
-- 	where pubNo not in (2,3);
...
-- 5) 논리 : and, or, xor, not
-- 저자가 성춘향이고 가격이 20000 이상인 도서 조회
select * from book
	where bookAuthor = '성춘향' and bookPrice >= 20000;

-- 저자가 성춘향 또는 가격이 20000 이상인 도서 조회 (공통된 조건 포함)
select * from book
...
-- 6) 패턴매칭 : like '패턴'  | not like 
-- [와일드 카드 문자]
--  % : 0개 이상의 문자를 가진 문자열
--  _ : 단일문자 (_ 개수 만큼의 문자로 구성)
        
--    문자열 패턴 예.
-- '홍%'  : '홍'으로 시작하는 문자열 검색
-- '%길%' : '길'을 포함하는 문자열 검색 ('길' 앞 뒤에 아무 문자열 포함 가능)
-- '%동' : '동'으로 끝나는 문자열 검색
-- '___' : 3개의 문자로 구성된 문자열
	
-- 도서테이블에서 '길동'이라는 이름을 가진 도서 검색
select * from book
...
-- book 테이블에서 도서명에 '데이터'가 들어 있지 않은 도서 조회
select * from book
